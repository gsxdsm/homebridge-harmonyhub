...
CommandAccessory.prototype._getCommandService = function(command) {
	...
	//this.log("Command ID", commandId, command)
        ...
        if (targetDevice){
		  ...
		  if (control){
	          ...
	          _.forEach(service.command.command_value.split(','), function(value) {
	            ...
	            .filter(function (action) { return action.label.toLowerCase() === value.toLowerCase()})
	            .pop()
	            if (controlFunction) {
				  //self.log("Control", controlFunction)
	              var encodedAction = controlFunction.action.replace(/\:/g, '::')
	              client.send('holdAction', 'action=' + encodedAction + ':status=press:timestamp=1')
	              client.send('holdAction', 'action=' + encodedAction + ':status=release:timestamp=2')
	            } else {
